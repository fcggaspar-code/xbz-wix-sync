...
async function createProductV1WithMedia(xbzProduct, fileId) {
  const name = getProductName(xbzProduct);
  const wixMediaUrl = `wix:image://v1/${fileId}/${fileId}#originWidth=800&originHeight=800`;
 
  // Usando Catalog V1 que permite imagem na criação
  const body = {
    product: {
      name: name,
      description: xbzProduct.descricao || name,
      productType: 'physical',
      price: 10.00,
      sku: `TESTE11-${xbzProduct.codigoXbz}`,
      visible: false,
      media: {
        items: [
          {
  ...
  console.log(`   🛒 Criando produto V1 com imagem: ${wixMediaUrl}`);
 
  const res = await request(
    `${CONFIG.WIX_API_BASE}/stores/v1/products`,
    {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${CONFIG.WIX_TOKEN}`,
  ...
  console.log(`   Wix V1 status: ${res.status}`);
  console.log(`   Resposta: ${res.body.substring(0, 400)}`);
 
  return { status: res.status, body: res.body };
}
 
async function sync() {
  console.log('🧪 TESTE v11 — Catalog V1 Create Product com mídia inclusa');
  ...
  try {
    ...
    for (const product of xbzProducts) {
      ...
      try {
        ...
        const result = await createProductV1WithMedia(product, fileId);
